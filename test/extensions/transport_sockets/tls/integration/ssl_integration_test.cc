...
TEST_P(SslCertficateIntegrationTest, ServerEcdsaClientRsaOnlyWithAccessLog) {
  ...
  if (tls_version_ == envoy::extensions::transport_sockets::tls::v3::TlsParameters::TLSv1_3) {
    EXPECT_THAT(log_result,
                StartsWith("DOWNSTREAM_TRANSPORT_FAILURE_REASON=TLS_error:_337645686:SSL_routines:"
                           "tls_choose_sigalg:no_suitable_signature_algorithm FILTER_CHAIN_NAME=-"));
  } else {
    EXPECT_THAT(log_result, StartsWith("DOWNSTREAM_TRANSPORT_FAILURE_REASON=TLS_error:_337092801:"
                                       "SSL_routines:tls_post_process_client_hello:no_shared_cipher FILTER_CHAIN_NAME=-"));
  }
}
